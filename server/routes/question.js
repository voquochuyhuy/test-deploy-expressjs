...
router.post("/", async function (req, res, next) {
  const { userID,type,lang,content,additionalInfomation, audioURL,photoURL,questionType,comments, featuredAnswer, vote, status } = req.body;
  const date = moment(new Date()).format('YYYY-MM-DD hh:mm:ss');
  const id = uuidv4();
  const queryString = `insert into question (id,userID, type, lang,content, additionalInfomation, audioURL,photoURL, questionType, comments, featuredAnswer, vote, status, createdAt, updatedAt) values('${id}','${userID}','${type}','${type}','${lang}','${content}','${additionalInfomation}','${audioURL}','${photoURL}','${questionType}','${comments}','${featuredAnswer}',${vote},'${status}','${date}','${date}');`;
